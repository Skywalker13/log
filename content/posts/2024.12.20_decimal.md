...
Qui ne sait jamais confronté aux problèmes d'arrondis avec les nombres réels ?
...
Tous ces résultats seraient très perturbants pour un mathématicien qui n'aurait
aucune connaissance en informatique. D'une certaine manière, c'est même
inquiétant si on souhaite utiliser un ordinateur comme calculateur en physique.
Il est donc très important de comprendre la moindre ce qui se passe dans la
machinerie et pourquoi le résultat de cette équation si trivial, n'est pas
exacte.
...
Le résultat de cette équation rend un nombre réel avec une période de 42
décimales. Naïvement on pourrait penser que le fait qu'il y ait une période,
forcément l'ordinateur ne peut pas contenir un nombre réel d'une longueur
infini. Mais cette explication n'est pas juste car ici on représente les nombres
en base 10 et pas en base 2. Prennons un exemple encore plus simple avec un
nombre non périodique et demandons à nos langages de programmation, ce qu'il en
est.
...
Vous pensez vraiment que tout est bon ? Hé bien il y a un piège. Tous ces
résultats sont apparemment correctes, mais pourtant ils apparaissent juste
...
Une nombre réel comme 0.1 doit être converti au format binaire avec une [virgule
flottante][1]. C'est une représentation numérique limitée par le nombre de bits
à disposition. Par exemple le type double utilise 64 bits.
...
Le nombre à virgule flottant se calcul ainsi :
...
Pour trouver le 1.6 (la mantisse) il faut effectuer une conversion
décimale-binaire en utilisant une méthode assez simple. On multiplie par 2 et on
prend la partie entière du résultat, puis on répète ce processus avec la partie
fractionnaire restante.
...
1.100110011001100110011…

Il est alors impossible d'avoir une représentation exacte en base 2 car nous
avons un nombre limité de bits à disposition. Un nombre qui nous semble très
...
précision ? Prennons un exemple tout simple. J'ai 3 amis à qui je dois partager
...
1. Je découpe le billet en trois parts égales avec une paire de ciseaux  
   Je doute que mes amis soient très content mais avec cette technique je peux
...
A part la première solution, les deux autres sont satisfaisantes dans ce
...
Chaque moi, il y a un montant de 10/3 CHF qui est ajouté à chaque compte. Si je
...
> de fois le professeur vous a dit que les résultats ne l'intéresse pas, c'est
> uniquement l'équation qui importe.

Admettons que je travail de manière traditionnel. Mon système stock les
résultats au centime et donc je partage le billet de 10 CHF en part de 3.33 CHF.
...
Le 1er octobre il y a 10 centimes qui n'ont jamais été distribués. Il faudrait
au moins distribuer encore 9 centimes. Le dernier centime, je peux le garder
dans ma poche.
...
une bonne précision. Avec l'arrondi final on a donc bien récupéré les 3 centimes
manquants. C'est mieux mais ça ne suffit pas. On ne fait que repousser le
problème en augmentant la précision.

Prennons par exemple cette équation :
...
virgule flottantes, alors comment pouvons-nous régler cette question une bonne
fois pour toute. Un moyen simple de ne jamais perdre en précision c'est de tout
simplement stocker vos nombres sous la forme de chaînes de caractères. Revenons
à notre exemple avec 0.1. Si je stock `"0.1"` au lieu du double 0.1, je donne
...
précision car la période est représentée par les parenthèses.

Je ne sais pas pour vous, mais pour moi ce n'est pas complètement satisfaisant.
Bien que les nombres soient exactes, c'est surtout le second cas qui me dérange
un peu. Et si on utilisait des représentation fractionnaires ? Pour 0.1, on
...
Je pense que vous êtres d'accord avec moi. Utiliser `"1/98"` au lieu de
...
rationnels, alors le numérateur et le dénominateur seront toujours des entiers.
En informatique, il n'y a jamais de perte de précision avec les nombres entiers.
Le seul problème vient du nombre de bits à disposition. Mais je vous rassure,
avec des nombres entier 64 bits, on est tranquille pour toutes nos applications.
...
Prennons l'exemple suivant où on verse 10 fois 10/3 CHF.
...
Il va falloire demander un arrondi au centime.

```js
new Fraction(10).mul("10/3").round(2).toString(); // '33.33'
```

A ce moment là, on a effectivement un montant plausible en centime. Reste que
...
le résultat final. En ayant la valeur exacte ainsi on peut retrouver (ou mieux
...
Pour celà, j'ai imaginé [un petit projet][8] qui exploite [Fraction.js][2] et
...
terminer on divise ce résultat par 100. Dit autrement, on a calculé le 8.1% de
4.2 qui donne bien 0.3402.

Etant donné qu'on veut travailler en centime, on peut demander un arrondi de
cette manière :
...
Cette petite bilbiothèque permet bien entendu de faire l'inverse. On peut donner
une version sérrialisée afin d'y retrouver un object de type `Fric` qui nous
...
Il y a effectivement des langages qui intègre un "nouveau" type pour régler la
problématique de la représentation sous la forme de virgule flottante. Mais
parfois ce n'est que repousser le problème en augmentant simplement la précision
comme en C#. Et malheureusement il est assez facile de tomber sur des cas
impossible à représenter correctement même avec ce genre de type.
...
très intéressant. Ce n'est peut être pas étrangé à sa grande popularité dans le
...
Certainement qu'il faut bien réfléchir aux types à utiliser avant de se lancer
dans des calculs financiers ou scientifiques. Le choix dépend aussi des
performances. Si les performances ne sont pas indispensables, je pense qu'opter
pour une représentation fractionnaire et une très bonne idée. Gardez les
...
nombre `Fric` qui conservent l'historique des opérations.
