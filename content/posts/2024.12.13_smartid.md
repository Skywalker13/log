...
date: 2024-12-13
tags: [database]
description: "ou l'importance des identifiants dans une base de donnée"
draft: false
---
...
[diagrammes ER][3], etc. Dans le fond, c'est toujours la même idée. On créer des
...
contentons-nous de quelques diagrammes très simples qui permettent de résumé
...
> Hein ? Ne jamais rien effacer ? Hé mec, cest n'importe quoi...
...
qu'on arrête sa flèche du temps. De cette manière on pourrait même le ressucité
un jour.
...
souhaîtable et ça demande potentiellement de la migration.
...
souhaîte pas polluer le modèle et encore moins le migrer. Pour cela, on peut
...
modèles. Nous avons utilisé ce principe pour les tags sur les entités. Ici, je
vais vous donner un exemple en production. Pour moi (et pas que moi), il est
exclu de toucher aux modèles des entités pour les polluer avec des tags.
...
`contact@unique`. Cette entité "entityTags" sert de table de relations. On
aurait pu nous arrêter là. Mais on a décidé d'aller plus loin.

Au lieu d'avoir une collection de `tag@` dans l'entité "entityTags", nous avons
une collection de `tagLink@` (oui encore un type). Un "tagLink" se présente
ainsi :
...
très pratique. Avec ce système basé sur les SmartId, il est alors possible
d'assigner des tags à n'importe quelle entité du système sans toucher au moindre
modèle et tout en pouvant assigner des données avec chaque tag posé. Finalement
le schéma se construit avec les SmartId.
...
(par exemple). Le système créer des clients sous la forme `customer@uuid`. Le
système externe créer des clients avec des nombres auto-incrémentés. Client
1001, 1002, 1005, 1011, ... Un système traditionnel... Quand nous importons
depuis ce système, nous pouvons utiliser les SmartId ainsi (par exemple) :
...
Si je ne vous ai pas convaincu ça n'a pas d'importance, mais tant mieux si ça
peut vous faire réfléchir. Moi je lui suis et depuis lors je n'ai plus aucune
envie de faire de la modélisation à la façon de grand-papa.
