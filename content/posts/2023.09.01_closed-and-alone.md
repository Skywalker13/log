...
date: 2023-09-01
tags: [c]
description: "… du code bugué que personne ne regarde"
draft: false
---
...
excellent cas d'école. Cela montre aussi l'importance d'avoir toujours au moins
...
de `if` qui semblent suivre la pensée du développeur qui a écrit le code. Ce que
je veux dire c'est que (pour moi en tout cas) il y a deux grandes phases très
rapprochées à l'écriture de n'importe quel code source. D'abord il y a le
...
ce moment là. Bref, c'est juste du code bâclé (et surtout, bugué).
...
Juste en faisant cette inversion, quelque chose de bizarre apparaît. Quand
...
forcément sur un `return`; simplifions tout cela.
...
Bien, regardons ce bout de code. Il est un peu bizarre, vous ne trouvez pas ? Si
...
Et bien c'est un bug, quand [errno][2] vaut 1 cela signifie que l'erreur `EPERM`
...
réalité aussi buguée. Admettons qu'elle soit corrigée. Elle rend 0 ou 1 quand
...
Je vous épargne cette écriture pour cette fois. Néanmoins c'est une façon tout à
fait reconnue par les experts en C pour produire un booléen.
